...
public class ProductListLoader extends AsyncTaskLoader<List<ProductObject>> {

    public static final String PRODUCT_SOUP = "products";
     ...
     */
    public ProductListLoader(Context context, UserAccount account) {
        super(context);
    ...
    @Override
    public List<ProductObject> loadInBackground() {
        if (!smartStore.hasSoup(PRODUCT_SOUP)) {
            return null;
        }
        final QuerySpec querySpec = QuerySpec.buildAllQuerySpec(PRODUCT_SOUP,
                ProductObject.PRODUCT_CODE, QuerySpec.Order.ascending, LIMIT);
        JSONArray results = null;
        List<ProductObject> products = new ArrayList<ProductObject>();
        try {
            results = smartStore.query(querySpec, 0);
            for (int i = 0; i < results.length(); i++) {
                products.add(new ProductObject(results.getJSONObject(i)));
            }
